...
- Overrides (optional):
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_CACHE_PROBE=1` to run a fresh Claude CLI turn plus two native resumes and require at least 95% prompt-cache reuse on the second resume. This probe disables the image, MCP, and model-switch probes.
...
- `pnpm test:docker:live-cli-backend:claude:cache` requires Anthropic API-key auth. It logs normalized cache usage for both resumes and fails when the second resume falls below 95% reuse.
