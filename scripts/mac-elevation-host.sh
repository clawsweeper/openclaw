...
resolve_migration_inputs() {
  ...
  if [[ -n "$inline_config" ]]; then
    case "$inline_config" in /*) ;; *) fail 'migration LaunchAgent OPENCLAW_CONFIG_PATH must be absolute' ;; esac
    if [[ "$CONFIG_PATH_EXPLICIT" == "1" && "$CONFIG_PATH" != "$inline_config" ]]; then
...
resolve_adoption_inputs() {
  ...
  refresh_runtime_paths
  OPENCLAW_CLI=("$(command -v openclaw 2>/dev/null || true)")
  local app_binary="$APP_PATH/Contents/MacOS/OpenClaw" pid command_line executable matches=()
  while IFS= read -r pid; do
    ...
    case "$command_line" in
      "$app_binary --attach-only --background-only"|"$app_binary --background-only --attach-only")
        ADOPTION_ATTACH_ONLY=1 ;;
      "$app_binary --background-only") ADOPTION_ATTACH_ONLY=0 ;;
      *) continue ;;
    esac
    matches+=("$pid")
  done < <(pgrep -x OpenClaw 2>/dev/null || true)
  [[ "${#matches[@]}" == "1" ]] || fail 'adoption requires exactly one unsupervised background-only OpenClaw process'
  ADOPTION_PID="${matches[0]}"
}
...
install_host() {
  ...
  if [[ -n "$MIGRATION_LABEL" ]]; then
    migration_pid="$(job_pid_for_domain "$launch_domain/$MIGRATION_LABEL")"
    [[ "$(shasum -a 256 "$MIGRATE_LAUNCH_AGENT" | awk '{print $1}')" == "$ROLLBACK_MIGRATION_PLIST_SHA" ]] ||
...
recover_install() {
  ...
  if [[ -n "$ROLLBACK_MIGRATION_SOURCE" && -f "$ROLLBACK_MIGRATION_PLIST" ]]; then
    if [[ "$CUTOVER_MIGRATION_REMOVED" == "1" ]]; then
      cp -p "$ROLLBACK_MIGRATION_PLIST" "$ROLLBACK_MIGRATION_SOURCE" || recovery_failed=1
    ...
    [[ "$restored_state" != 'unknown' ]] || recovery_failed=1
    if [[ "$ROLLBACK_MIGRATION_WAS_LOADED" == "1" && "$restored_state" == 'absent' ]]; then
  ...
  if [[ "$CUTOVER_ADOPTION_STOPPED" == "1" && "$recovery_failed" == "0" ]]; then
    if [[ "$ROLLBACK_ADOPTED_APP_ATTACH_ONLY" == "1" ]]; then
      open -n "$APP_PATH" --args --attach-only --background-only >/dev/null 2>&1 || recovery_failed=1
    else
